...
class LibraryAdminPanel(ctk.CTk):
    ...
    def delete_member(self):
        ...
        if confirm:
            try:
                with db_connection() as conn:
                    cursor = conn.cursor()
                    cursor.execute("DELETE FROM user_Collections WHERE user_id = %s", (member_id,))
    ...
    def update_member(self):
        ...
        # Password entry field that hides input
        password_entry = ctk.CTkEntry(form_frame, placeholder_text="Password", show="*")
        password_entry.pack(pady=5)
        password_entry.insert(0, member_data[7])  # Initial value should remain hidden
        ...
        show_password_checkbox = ctk.CTkCheckBox(form_frame, text="Show Password", variable=show_password_var, command=toggle_password_visibility)
